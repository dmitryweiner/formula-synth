...
// Приём общеизвестен (swevans/unmute, feross/unmute-ios-audio); реализован
// инлайн, без рантайм-зависимостей. Ненадёжен на 100% (ограничение платформы),
// но закрывает самый массовый случай «жму Play — тишины».
...
export class IosAudioUnlock {
  ...
  // ВАЖНО: вызывать СИНХРОННО внутри пользовательского жеста (клик Play), до
  // любого await — иначе iOS не считает это активацией и приём не сработает.
  play(): void {
    if (!this.el) {
